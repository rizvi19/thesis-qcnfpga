...
valid, both candidates become feasible. A scarcity-oriented profile may then prefer
the larger bottleneck reserve of $\mathcal R_1$; a path-cost-oriented profile may
prefer the two-hop $\mathcal R_0$. Thus feasibility answers ``may this route be
used?'', whereas the profile answers ``which permitted route best expresses the
current operating preference?'' No finite score can override $v_i=0$.
...
The registered profile words also preserve four half-unit coefficients
$\boldsymbol\alpha^{(k)}$. Their conceptual role is to parameterize an upstream
local/link-cost construction. In the completed physical shell, candidate path cost
arrives already computed under the common replay; the candidate evaluator consumes
only the profile word's lower six bits, namely the three Tchebycheff ratios.
Therefore the physical H2--H4 comparison changes the relative emphasis among path
cost, route-quality deficit, and utilization imbalance. It does not recompute an
$\boldsymbol\alpha$-weighted graph path inside the FPGA.
...
\end{align}
so $\mathcal R_0$ wins by 96 score units. For scarcity-protection profile $\Pi_1$
the deployed ratio is $(1,1,2)$:
\begin{align}
S_0^{(1)}&=\max(0,65485,2\cdot65527)=131054,\\
S_1^{(1)}&=\max(65533,0,0)=65533,
\end{align}
so $\mathcal R_1$ wins. In this contract, the registered profile name identifies a
controller mode; the physical arithmetic is defined by the numerical ratio. If two
scores tie, comparison continues lexicographically by raw path cost, hop count, and
candidate slot, guaranteeing one reproducible result.
