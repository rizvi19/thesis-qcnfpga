...
% \node[ext] (kms)
%   {KMS/SDN monitoring\\and policy};

% \node[ext, right=of kms] (gen)
%   {candidate generation\\and hard checks};

% \node[core, below=of gen] (fpga)
%   {QFlow profile selection\\and fixed-point evaluation};

% \node[ext, left=of fpga] (install)
%   {authorization, installation,\\and fallback};

% \draw[arr]
%   (kms) -- node[arrlabel, above] {state/metadata} (gen);

% \draw[arr]
%   (gen) -- node[arrlabel, right, text width=20mm]
%   {bounded\\descriptors} (fpga);

% \draw[arr]
%   (fpga) -- node[arrlabel, below] {result/status} (install);

% \draw[arr]
%   (install) -- node[arrlabel, left] {telemetry} (kms);
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  % Increased slightly to 16mm. \resizebox will still ensure it fits the page perfectly.
  node distance=16mm,
  ext/.style={
    draw=QFlowGray,
    rounded corners,
  ...
  arrlabel/.style={
    font=\scriptsize,
    align=center
    % Removed 'fill=white' so the text box doesn't have an opaque background
  }
]

% Place nodes horizontally in a single row
\node[ext] (kms)
  {KMS/SDN\\monitoring\\and policy};

\node[ext, right=of kms] (gen)
  {candidate gen-\\eration and\\hard checks};

\node[core, right=of gen] (fpga)
  {QFlow profile\\selection and fixed-\\point evaluation};

\node[ext, right=of fpga] (install)
  {authorization, in-\\stallation, fallback};

% Draw straight horizontal arrows, placing the labels ABOVE the line and split into two lines
\draw[arr]
  (kms) -- node[arrlabel, above] {state/\\metadata} (gen);

\draw[arr]
  (gen) -- node[arrlabel, above] {bounded\\descriptors} (fpga);

\draw[arr]
  (fpga) -- node[arrlabel, above] {result/\\status} (install);

% Draw the long curved return arrow, placing the label BELOW the curve
\draw[arr]
  (install.south) to[bend left=20] node[arrlabel, below] {telemetry} (kms.south);

\end{tikzpicture}%
}
\caption{System boundary. Only profile selection and candidate evaluation are included in the measured FPGA kernel.}
\label{fig:system-boundary}
\end{figure}
...
\begin{tikzpicture}[
  ...
  ctrl/.style={draw=QFlowAmber,rounded corners,fill=QFlowLightAmber,minimum height=10mm,
    text width=27mm,align=center,font=\scriptsize},
  arr/.style={-{Latex[length=2mm]},thick,QFlowNavy},
  darr/.style={<->, >={Latex[length=2mm]},thick,QFlowNavy},
...
% Split nodes placed diagonally
\node[mem, above right=2mm and 8mm of radix] (prom) {H4 policy ROM\\256$\times$2 b};
\node[ctrl, below right=2mm and 8mm of radix] (rule) {H3 first-match\\rule controller};

% Dwell placed symmetrically after the split
\node[ctrl, below right=2mm and 8mm of prom] (dwell) {H4 dwell control\\active profile 2 b};
\node[mem, right=of dwell] (prof) {profile ROM\\4$\times$18 b};

% 2. BOTTOM ROW
\node[block, below=20mm of prof] (score) {ratio products\\Tchebycheff max\\18-b score};

% Build leftwards from score
\node[block, left=of score] (norm) {range normalizer\\shared 49$\times$33 divider};
\node[block, left=of norm] (feas) {validity gating and range reduction\\min/max per objective};
\node[block, left=of feas] (store) {candidate storage/stream\\valid, cost, quality, util., hops, slot};

% Build rightwards from score
\node[block, right=of score] (win) {winner/tie registers\\score, cost, hops, slot};

% OVERRIDE: Widened result block to catch the left-shifted vertical arrow
\node[block, right=of win, text width=32mm] (result) {status/result registers\\policy, state, profile, path, score, quality};

% 3. FSM & FORMATTER (Bottom Control Loops)
\node[ctrl, below=12mm of norm] (fsm) {control FSM\\iteration, bypass, handshakes};
\node[ctrl, right=of fsm] (count) {16-b kernel cycle counter};

% OVERRIDE: Narrowed formatter block so the vertical arrow passes safely on its left
\node[block, text width=24mm] (fmt) at (fsm -| result) {QF6R formatter + UART\\outside kernel cycles};

% --- Standard Forward Paths ---
\draw[arr] (in)--(quant);
\draw[arr] (quant)--(radix);

% Split routing
\draw[arr] (radix.east) -- (prom.west);
\draw[arr] (radix.east) -- (rule.west);
\draw[arr] (prom.east) -- (dwell.west);
\draw[arr] (rule.east) -- (dwell.west);
...
% --- Cross-Row and Feedback Routing ---
\draw[arr] (prof)--(score);

% FSM loops
\draw[arr] (fsm.west) -| ([xshift=5mm]store.south);
\draw[arr] (fsm)--(norm);
\draw[arr] (fsm)--(count);

% FIX: Feedback shifted far left (-14mm) to clear the formatter's left edge
\draw[arr] (count.east) -| ([xshift=-16mm]result.south);

% FIX: Bi-directional formatter arrow shifted right (+5mm) to prevent crowding
\draw[darr] ([xshift=5mm]result.south) -- ([xshift=5mm]fmt.north);
...
fixed-point reference; panel (b) shows the uncropped photograph of the board used.
