...
\end{tikzpicture}}
\caption{System context of \QFlow. Candidate generation and route installation are
control-plane functions outside the FPGA kernel.}
...
\end{equation}
Each candidate $\mathcal{R}_i$ has a fixed-width descriptor containing feasibility,
distance or consumption cost, key-resource conditions, load-related indicators, and
a deterministic identifier. \QFlow\ selects a scoring profile, evaluates the
descriptors, and returns the chosen identifier, score, profile, and status.
...
quantization, read-only policy lookup, weighted fixed-point scoring, feasibility
checks, comparison, and deterministic tie-breaking. These operations are suitable
...
chandra2025synchronization}. Within the documented literature audit, no directly
...
\begin{enumerate}
  \item rejects infeasible candidates according to explicit hard constraints;
...
\begin{enumerate}
  ...
  \item define a fixed-point candidate-scoring contract with distinct hard
  eligibility and soft preference;
...
\begin{description}[leftmargin=1.3cm,style=nextline]
  ...
  \item[\textbf{RH2.}] Every checked physical result field will match the frozen
...
\begin{enumerate}
  ...
  hard eligibility from profile-conditioned multi-objective preference;
  \item an adaptive controller with four radix-4 features, 256 states, four profile
  actions, offline tabular Q-learning, a 256-entry two-bit policy ROM, and
  minimum-dwell control;
