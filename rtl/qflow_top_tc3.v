...
module qflow_top_tc3 #(
    ...
    skag_mem_tc3 #(.ADDR_W(ADDR_W), .DEPTH(1 << ADDR_W)) u_skag (
        .clk(clk), .rst_n(rst_n), .cfg_we_i(cfg_we_i), .cfg_addr_i(cfg_addr_i), .cfg_wdata_i(cfg_wdata_i),
        .fdpe_upd_valid_i(fdpe_valid_w), .fdpe_addr_i(fdpe_addr_d2), .fdpe_fidelity_i(fdpe_fidelity_w),
