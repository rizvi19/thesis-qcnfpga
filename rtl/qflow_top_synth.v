...
module qflow_top_synth #(
    ...
    // PMO-GA multigen candidate stream
    ...
    localparam [15:0] ALPHA1_Q8_8 = 16'd256; // 1.0
    localparam [15:0] ALPHA2_Q8_8 = 16'd384; // 1.5
    localparam [15:0] ALPHA3_Q8_8 = 16'd128; // 0.5
    localparam [15:0] ALPHA4_Q8_8 = 16'd512; // 2.0
    ...
    xorshift128plus u_prng (
        ...
        .seed0_i(seed0_i),
        .seed1_i(seed1_i),
        .enable_i(prng_enable_i),
        .rand_o(rand_o),
        .valid_o(rand_valid_o),
        .seeded_o(seeded_o)
    );
    ...
    fdpe u_fdpe (
        .clk(clk),
        .rst_n(rst_n),
        .valid_i(fdpe_launch_i),
        .x_q4_13_i(fdpe_x_q4_13_i),
        .f_init_q016_i(fdpe_f_init_q016_i),
        .valid_o(fdpe_valid_w),
        .fidelity_q016_o(fdpe_fidelity_w)
    );

    assign fdpe_result_valid_o = fdpe_valid_w;
    assign fdpe_result_q016_o  = fdpe_fidelity_w;

    // Delay the FDPE address to align with fdpe_valid_w.
    reg [ADDR_W-1:0] fdpe_addr_d0, fdpe_addr_d1, fdpe_addr_d2;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (fdpe_launch_i)
                fdpe_addr_d0 <= fdpe_addr_i;
            fdpe_addr_d1 <= fdpe_addr_d0;
            fdpe_addr_d2 <= fdpe_addr_d1;
        end
    end

    // SKAG memory/update block (optimized BRAM-oriented version)
    wire [31:0] skag_fdpe_weight_w;

    skag_mem_bram #(.ADDR_W(ADDR_W), .DEPTH(1 << ADDR_W)) u_skag (
        .clk(clk),
        .rst_n(rst_n),
        .cfg_we_i(cfg_we_i),
        .cfg_addr_i(cfg_addr_i),
        .cfg_wdata_i(cfg_wdata_i),
        .fdpe_upd_valid_i(fdpe_valid_w),
        .fdpe_addr_i(fdpe_addr_d2),
        .fdpe_fidelity_i(fdpe_fidelity_w),
        .alpha1_q8_8_i(ALPHA1_Q8_8),
        .alpha2_q8_8_i(ALPHA2_Q8_8),
        .alpha3_q8_8_i(ALPHA3_Q8_8),
        .alpha4_q8_8_i(ALPHA4_Q8_8),
        .ga_rd_en_i(skag_ga_rd_en_i),
        .ga_rd_addr_i(skag_ga_rd_addr_i),
        .ga_rd_edge_o(skag_ga_edge_o),
        .ga_rd_weight_o(skag_ga_weight_o),
        .ga_rd_valid_o(skag_ga_valid_o),
        .fdpe_done_o(fdpe_done_o),
        .fdpe_weight_o(skag_fdpe_weight_w)
    );

    // PMO-GA multigeneration kernel
    pmo_ga_multigen #(.INDEX_W(4), .GEN_W(4)) u_pmo_ga (
        .clk(clk),
        .rst_n(rst_n),
        .start_i(ga_start_i),
        .pop_size_i(ga_pop_size_i),
        .gen_count_i(ga_gen_count_i),
        .cand_valid_i(cand_valid_i),
        .cand_id_i(cand_id_i),
        .cand_latency_i(cand_latency_i),
        .cand_fidelity_i(cand_fidelity_i),
        .cand_util_i(cand_util_i),
        .cand_gene_i(cand_gene_i),
        .req_valid_o(req_valid_o),
        .req_gen_o(req_gen_o),
        .req_index_o(req_index_o),
        .result_valid_o(ga_result_valid_o),
        .best_id_o(ga_best_id_o),
        .best_latency_o(ga_best_latency_o),
        .best_fidelity_o(ga_best_fidelity_o),
        .best_util_o(ga_best_util_o),
        .final_child_gene_o(ga_child_gene_o),
        .final_mutated_gene_o(ga_mutated_gene_o)
    );
