...
`endif
    ...
    begin
        ...
        if ((key_count == 16'd0) || (fidelity == 16'd0) || (arrival_rate == 16'd0)) begin
            ...
            sum = t1 + t2 + t3 + t4;
            compute_weight = (sum[63:32] != 32'd0) ? INF_WEIGHT : sum[31:0];
        end
    end
    endfunction

    wire [15:0] fidelity_wdata = a_use_cfg ? cfg_wdata_i[31:16] : fdpe_s2_new_fidelity;
    wire [31:0] weight_cfg_wdata = compute_weight(cfg_wdata_i, alpha1_q8_8_i, alpha2_q8_8_i,
                                                  alpha3_q8_8_i, alpha4_q8_8_i);
    wire [31:0] weight_wdata = a_use_cfg ? weight_cfg_wdata : fdpe_s2_new_weight;
...
`ifndef SYNTHESIS
    always @(posedge clk) begin
        if (mem_we)
            fidelity_mem[mem_waddr] <= fidelity_wdata;
        if (ga_rd_en_i)
    ...
    always @(posedge clk) begin
        if (mem_we)
            weight_mem[mem_waddr] <= weight_wdata;
        if (ga_rd_en_i)
            b_weight_dout <= weight_mem[ga_rd_addr_i];
    end
`else
