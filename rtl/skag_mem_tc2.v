...
`endif
    ...
    wire a_use_fdpe_write = fdpe_s2_valid;
    wire a_use_fdpe_read  = (!fdpe_s0_valid) && (!fdpe_s1_valid) && (!fdpe_s2_valid) && fdpe_upd_valid_i;
    wire a_use_cfg        = (!a_use_fdpe_write) && (!a_use_fdpe_read) && cfg_we_i;
    wire [ADDR_W-1:0] mem_waddr = a_use_cfg ? cfg_addr_i : fdpe_s2_addr;
    ...
    function [31:0] combine_weight;
        ...
        reg [63:0] sum;
    begin
        if ((base_term == INF_WEIGHT) || (fidelity == 16'd0)) begin
            combine_weight = INF_WEIGHT;
        end else begin
            dyn = compute_fidelity_term(fidelity, a2_q8_8);
            if (dyn == INF_WEIGHT) combine_weight = INF_WEIGHT;
            else begin
                sum = {32'd0, base_term} + {32'd0, dyn};
                combine_weight = (sum[63:32] != 32'd0) ? INF_WEIGHT : sum[31:0];
            end
        end
    end
    endfunction
    ...
    wire [31:0] cfg_weight  = combine_weight(cfg_base, cfg_fidelity, alpha2_q8_8_i);

    wire [15:0] fidelity_wdata = a_use_cfg ? cfg_fidelity : fdpe_s2_new_fidelity;
    wire [31:0] weight_wdata   = a_use_cfg ? cfg_weight   : fdpe_s2_new_weight;
...
`endif
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            fdpe_s1_valid <= 1'b0; fdpe_s1_addr <= {ADDR_W{1'b0}}; fdpe_s1_new_fidelity <= 16'd0; fdpe_s1_new_weight <= INF_WEIGHT;
            ...
            fdpe_done_o <= 1'b0; ga_rd_valid_o <= 1'b0;
            ...
            if (fdpe_s0_valid) begin
                dynamic_weight_v = combine_weight(fdpe_s0_base, fdpe_s0_fidelity, alpha2_q8_8_i);
                fdpe_s1_valid <= 1'b1;
                fdpe_s1_addr  <= fdpe_s0_addr;
                fdpe_s1_new_fidelity <= fdpe_s0_fidelity;
                fdpe_s1_new_weight   <= dynamic_weight_v;
            end else begin
                fdpe_s1_valid <= 1'b0;
