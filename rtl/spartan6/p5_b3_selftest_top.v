...
module p5_b3_selftest_top #(
    ...
    reg failed;
    reg all_done;
    reg [114:0] active_vector;
    wire [114:0] selected_vector;
    wire core_ready;
    wire core_busy;
    wire core_done;
    wire core_feasible;
    wire [31:0] core_weight;
    wire [15:0] display_value;

    function [114:0] board_vector;
        input [4:0] vector_index;
        begin
            case (vector_index)
`include "p5_b3_board_vectors.vh"
                default: board_vector = 115'd0;
            endcase
        end
    endfunction
    ...
    assign display_value = all_done ? {8'hB3, pass_count} : {8'hB3, 3'b000, test_index};

    p5_b3_skag u_skag (
        .clk(clk_100mhz), .rst(btn_reset), .start(core_start),
        .edge_entry(active_vector[114:51]),
        .profile_payload(active_vector[50:33]),
        .ready(core_ready), .busy(core_busy), .done(core_done),
        .feasible(core_feasible), .weight_uq16_16(core_weight)
    );

    p5_b1_sevenseg u_display (
        .clk(clk_100mhz), .rst(btn_reset), .value(display_value),
    ...
    always @(posedge clk_100mhz) begin
        if (btn_reset) begin
            ...
            if (!all_done) begin
                ...
                if (core_done) begin
                    waiting_for_done <= 1'b0;
                    if ((core_weight == active_vector[32:1]) &&
                        (core_feasible == active_vector[0]))
