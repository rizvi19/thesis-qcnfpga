...
PLATFORM="sky130hd"
DESIGN="qflow_fdpe_v3"
VARIANT="base"

SRC_BASE="$FLOW_DIR"
OUT_DIR="$ROOT_DIR/results/partC_vlsi/openroad_fdpe_v3"
...
copy_if_exists "$SRC_BASE/designs/src/$DESIGN/fdpe_kernel_v3_lut64_interp.v" "$OUT_DIR/config/fdpe_kernel_v3_lut64_interp.v"
copy_if_exists "$SRC_BASE/designs/src/$DESIGN/exp_lut_64.hex" "$OUT_DIR/config/exp_lut_64.hex"
...
cat > "$OUT_DIR/openroad_fdpe_v3_status.md" <<'EOM'
...
echo "[PASS] wrote $OUT_DIR/openroad_fdpe_v3_status.md"
echo "[PASS] FDPE-V3 OpenROAD artifacts collected into $OUT_DIR"
