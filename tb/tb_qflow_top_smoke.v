...
module tb_qflow_top_smoke;
    ...
    initial begin
        ...
        if (!$value$plusargs("VECTORS=%s", vectors_file))
            vectors_file = "tb/qflow_top_candidates.memh";
        if (!$value$plusargs("LOG=%s", log_file))
            log_file = "results/phase7/qflow_top_smoke_sim_results.csv";
        if (!$value$plusargs("SUMMARY=%s", summary_file))
            summary_file = "results/phase7/qflow_top_smoke_sim_summary.txt";

        $display("[TB_QFLOW_TOP] vectors=%0s", vectors_file);
        $display("[TB_QFLOW_TOP] log=%0s", log_file);
        $display("[TB_QFLOW_TOP] summary=%0s", summary_file);
        ...
        if (log_fd == 0 || summary_fd == 0) begin
            $display("[TB_QFLOW_TOP] ERROR: could not open output files.");
            $fatal;
        end
        ...
        if (!fdpe_result_valid) begin
            $display("[TB_QFLOW_TOP] FAIL: timeout waiting for FDPE result.");
            $fatal;
        end
        ...
        if (!fdpe_done) begin
            $display("[TB_QFLOW_TOP] FAIL: timeout waiting for SKAG commit.");
            $fatal;
        end
        ...
        if (!skag_ga_valid) begin
            $display("[TB_QFLOW_TOP] FAIL: timeout waiting for SKAG GA read.");
            $fatal;
        end
        ...
        if (!ga_result_valid) begin
            $display("[TB_QFLOW_TOP] FAIL: timeout waiting for PMO-GA multigen result.");
            $fatal;
        end
        ...
        if (fail_count == 0) begin
            $display("[TB_QFLOW_TOP] PASS: fdpe=%0d skag_weight=0x%08x best_id=%0d child_gene=0x%04x mutated_gene=0x%04x",
                     captured_fdpe_result, captured_skag_weight, ga_best_id, ga_child_gene, ga_mutated_gene);
        end else begin
            $display("[TB_QFLOW_TOP] FAIL: mismatches=%0d", fail_count);
            $fatal;
        end
        $finish;
    end
