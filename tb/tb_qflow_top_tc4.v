...
module tb_qflow_top_tc4;
    ...
    initial begin
        ...
        if (!$value$plusargs("VECTORS=%s", vectors_file))
            vectors_file = "tb/qflow_top_tc4_candidates.memh";
        if (!$value$plusargs("LOG=%s", log_file))
            log_file = "results/phase9f/qflow_top_tc4_smoke_results.csv";
        if (!$value$plusargs("SUMMARY=%s", summary_file))
            summary_file = "results/phase9f/qflow_top_tc4_smoke_summary.txt";

        $display("[TB_QFLOW_TC4] vectors=%0s", vectors_file);
        $display("[TB_QFLOW_TC4] log=%0s", log_file);
        $display("[TB_QFLOW_TC4] summary=%0s", summary_file);
        ...
        if (log_fd == 0 || summary_fd == 0) begin
            $display("[TB_QFLOW_TC4] ERROR: could not open output files.");
            $fatal;
        end
        ...
        if (!fdpe_result_valid) begin
            $display("[TB_QFLOW_TC4] FAIL: timeout waiting for FDPE result.");
            $fatal;
        end
        ...
        timeout_ctr = 0;
        while (!fdpe_done && timeout_ctr < 120) begin
            @(posedge clk);
            timeout_ctr = timeout_ctr + 1;
        end
        if (!fdpe_done) begin
            $display("[TB_QFLOW_TC4] FAIL: timeout waiting for SKAG commit.");
            $fatal;
        end
        ...
        if (!skag_ga_valid) begin
            $display("[TB_QFLOW_TC4] FAIL: timeout waiting for SKAG GA read.");
            $fatal;
        end
        ...
        if (!ga_result_valid) begin
            $display("[TB_QFLOW_TC4] FAIL: timeout waiting for PMO-GA multigen result.");
            $fatal;
        end
        ...
        if (fail_count == 0) begin
            $display("[TB_QFLOW_TC4] PASS: fdpe=%0d skag_weight=0x%08x best_id=%0d child_gene=0x%04x mutated_gene=0x%04x",
                     captured_fdpe_result, captured_skag_weight, ga_best_id, ga_child_gene, ga_mutated_gene);
        end else begin
            $display("[TB_QFLOW_TC4] FAIL: mismatches=%0d", fail_count);
            $fatal;
        end
        $finish;
    end
