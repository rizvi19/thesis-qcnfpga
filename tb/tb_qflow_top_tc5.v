...
module tb_qflow_top_tc5;
    ...
    initial begin
        ...
        if (!$value$plusargs("VECTORS=%s", vectors_file))
            vectors_file = "tb/qflow_top_tc5_candidates.memh";
        if (!$value$plusargs("LOG=%s", log_file))
            log_file = "results/phase9f/qflow_top_tc5_smoke_results.csv";
        if (!$value$plusargs("SUMMARY=%s", summary_file))
            summary_file = "results/phase9f/qflow_top_tc5_smoke_summary.txt";

        $display("[TB_QFLOW_TC5] vectors=%0s", vectors_file);
        $display("[TB_QFLOW_TC5] log=%0s", log_file);
        $display("[TB_QFLOW_TC5] summary=%0s", summary_file);
        ...
        if (log_fd == 0 || summary_fd == 0) begin
            $display("[TB_QFLOW_TC5] ERROR: could not open output files.");
            $fatal;
        end
        ...
        timeout_ctr = 0;
        while (!fdpe_result_valid && timeout_ctr < 80) begin
            @(posedge clk);
            timeout_ctr = timeout_ctr + 1;
        end
        if (!fdpe_result_valid) begin
            $display("[TB_QFLOW_TC5] FAIL: timeout waiting for FDPE result.");
            $fatal;
        end
        ...
        timeout_ctr = 0;
        while (!fdpe_done && timeout_ctr < 160) begin
            @(posedge clk);
            timeout_ctr = timeout_ctr + 1;
        end
        if (!fdpe_done) begin
            $display("[TB_QFLOW_TC5] FAIL: timeout waiting for SKAG commit.");
            $fatal;
        end
        ...
        @(posedge clk);
        skag_ga_rd_en   <= 1'b1;
        skag_ga_rd_addr <= 12'd0;
        @(posedge clk);
        skag_ga_rd_en   <= 1'b0;
        skag_ga_rd_addr <= 12'd0;

        timeout_ctr = 0;
        while (!skag_ga_valid && timeout_ctr < 30) begin
            @(posedge clk);
            timeout_ctr = timeout_ctr + 1;
        end
        if (!skag_ga_valid) begin
            $display("[TB_QFLOW_TC5] FAIL: timeout waiting for SKAG GA read.");
            $fatal;
        end
        ...
        if (!ga_result_valid) begin
            $display("[TB_QFLOW_TC5] FAIL: timeout waiting for PMO-GA multigen result.");
            $fatal;
        end
        ...
        if (fail_count == 0) begin
            $display("[TB_QFLOW_TC5] PASS: fdpe=%0d skag_weight=0x%08x best_id=%0d child_gene=0x%04x mutated_gene=0x%04x",
                     captured_fdpe_result, captured_skag_weight, ga_best_id, ga_child_gene, ga_mutated_gene);
        end else begin
            $display("[TB_QFLOW_TC5] FAIL: mismatches=%0d", fail_count);
            $fatal;
        end
        $finish;
    end
