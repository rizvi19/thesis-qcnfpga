...
module tb_skag_bram;
    ...
    initial begin
        if (!$value$plusargs("VECTORS=%s", vectors_file))
            vectors_file = "tb/skag_bram_vectors.memh";
        if (!$value$plusargs("LOG=%s", log_file))
            log_file = "results/phase9a/skag_bram_sim_results.csv";
        if (!$value$plusargs("SUMMARY=%s", summary_file))
            summary_file = "results/phase9a/skag_bram_sim_summary.txt";

        $display("[TB_SKAG_BRAM] vectors=%0s", vectors_file);
        $display("[TB_SKAG_BRAM] log=%0s", log_file);
        $display("[TB_SKAG_BRAM] summary=%0s", summary_file);
        ...
        if (log_fd == 0) begin
            $display("[TB_SKAG_BRAM] ERROR: could not open log file.");
            $finish;
        end
        ...
        for (i = 0; i < vector_count; i = i + 1) begin
            ...
            if ((ga_rd_valid !== exp_ga_valid) ||
                ...
                $display("[TB_SKAG_BRAM][MISMATCH] cycle=%0d exp_valid=%0d obs_valid=%0d exp_edge=%h obs_edge=%h exp_weight=%h obs_weight=%h",
        ...
        $fdisplay(summary_fd, "SKAG_BRAM simulation summary");
        ...
        if (fail_count == 0)
            $display("[TB_SKAG_BRAM] PASS: vectors=%0d max_weight_diff=%0d", vector_count, max_weight_diff);
        else begin
            $display("[TB_SKAG_BRAM] FAIL: fails=%0d max_weight_diff=%0d", fail_count, max_weight_diff);
            $fatal(1, "[TB_SKAG_BRAM] Simulation failed.");
        end
