...
module tb_rl_controller_integrated;
    ...
    initial begin
        ...
        vector_file = $fopen("sim/rl/p4_integrated_trace_vectors.txt", "r");
        if (vector_file == 0) begin
            $display("P4_INTEGRATED_RTL_FAIL cannot_open_vector_file");
            $finish;
        end
        read_vector;
        while (scan_count == 22) begin
            ...
            if ((ready !== ready_i[0]) || (busy !== busy_i[0]) || (done !== done_i[0]) ||
                ...
                $display("P4_INTEGRATED_MISMATCH cycle=%0d", cycle_i);
            end
        ...
        if (scan_count != -1) begin
            $display("P4_INTEGRATED_RTL_FAIL malformed_vector_file scan_count=%0d", scan_count);
            $finish;
        end
        if (vector_count != 2125 || decision_count != 530) begin
            $display("P4_INTEGRATED_RTL_FAIL vectors=%0d decisions=%0d", vector_count, decision_count);
            $finish;
        end
        if (failure_count != 0) begin
            $display("P4_INTEGRATED_RTL_FAIL mismatches=%0d", failure_count);
            $finish;
        end
        $display("P4_INTEGRATED_RTL_PASS vectors=%0d decisions=%0d mismatches=0", vector_count, decision_count);
        $finish;
    end
endmodule
