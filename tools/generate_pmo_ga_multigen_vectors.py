...
# Deterministic 3-generation candidate set.
# Gen 0 keeps the known Ring-6 style winner at id=4.
# Gen 1 is slightly worse overall.
# Gen 2 introduces a better global elite at id=38.
DATA = [
    [
...
def main():
    ...
    csv_path = out_dir / 'pmo_ga_multigen_candidates.csv'
    memh_path = tb_dir / 'pmo_ga_multigen_vectors.memh'
    expected_path = out_dir / 'pmo_ga_multigen_expected.json'
    ...
    expected = {
        ...
        'population_size': POP,
        'expected_global_best_id': global_best['id'],
        'expected_global_best_latency_q16_16': global_best['lat'],
        'expected_global_best_fidelity_q0_16': global_best['fid'],
        'expected_global_best_util_q0_16': global_best['util'],
        'expected_last_generation_child_gene16': child,
        'expected_last_generation_mutated_gene16': mutated,
        'per_generation_best_ids': [r['id'] for r in per_gen_best],
        'notes': [
            'Step 6C validates multi-generation request sequencing and global elitism.',
            'The final best candidate should come from generation 2 with improved latency/fidelity/util tradeoff.'
        ]
    }
    expected_path.write_text(json.dumps(expected, indent=2))

    print(f'Wrote {len(rows)} PMO-GA multigen candidates to {memh_path}')
    print(f' - {csv_path}')
