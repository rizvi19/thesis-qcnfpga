...
def main():
    ...
    csv_path = out_dir / 'qflow_top_tc5_candidates.csv'
    memh_path = tb_dir / 'qflow_top_tc5_candidates.memh'
    expected_path = out_dir / 'qflow_top_tc5_expected.json'
    ...
    expected = {
        'integration_stage': 'step9e_top_tc5_smoke',
        ...
        'notes': [
            'Step 9G smoke test validates the timing-closure-5 top using the pipelined FDPE plus tc4 SKAG integration.',
            'Candidate stream remains deterministic for controlled integration checking.'
        ]
    }
    expected_path.write_text(json.dumps(expected, indent=2) + "\n")

    print(f'Wrote {len(rows)} top-level timing-closure-5 candidates to {memh_path}')
    print(f' - {csv_path}')
