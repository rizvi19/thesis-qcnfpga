...
def main():
    ...
    csv_path = out_dir / 'qflow_top_tc3_candidates.csv'
    memh_path = tb_dir / 'qflow_top_tc3_candidates.memh'
    expected_path = out_dir / 'qflow_top_tc3_expected.json'
    ...
    expected = {
        'integration_stage': 'step9e_top_tc3_smoke',
        ...
        'notes': [
            'Step 9E smoke test validates the timing-closure-3 top using the deeper SKAG FDPE update pipeline.',
            'Candidate stream remains deterministic for controlled integration checking.'
        ]
    }
    expected_path.write_text(json.dumps(expected, indent=2) + "\n")

    print(f'Wrote {len(rows)} top-level timing-closure-3 candidates to {memh_path}')
    print(f' - {csv_path}')
