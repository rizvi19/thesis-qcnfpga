...
class SkagModel:
    ...
    def cycle(self, vec: Vector):
        # Outputs from previous pending GA request / current memories
        if self.ga_pending:
        ...
        # New stage0 from current vector using old memories (matches RTL)
        if vec.fdpe_valid:
        ...
        # Config write unless same-cycle Port A conflict
        ...
        # Capture GA request for next cycle
...
def base_vectors() -> List[Vector]:
    e3 = pack_edge(10, 0xF100, 0x0800, 0x07AE)  # qber ~0.03, lambda=8.0
    e5 = pack_edge(8,  0xE800, 0x0600, 0x051E)  # qber ~0.02, lambda=6.0
    e7 = pack_edge(0,  0xD000, 0x0400, 0x028F)  # zero key_count -> INF
    e9 = pack_edge(6,  0xD800, 0x0000, 0x028F)  # zero arrival -> INF
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate SKAG golden vectors and memh stimulus.")
    parser.add_argument("--out-dir", type=Path, default=Path("results/phase2"))
    ...
    memh_path = tb_dir / "skag_vectors.memh"
    memh_path.write_text("\n".join(memh_lines) + "\n")

    csv_path = out_dir / "skag_golden_vectors.csv"
    ...
    summary = {
        ...
        "notes": [
            "Port A FDPE update uses 3-cycle pipeline.",
            "Port B GA read has 1-cycle latency.",
            "Same-address GA read during active FDPE update is stalled/dropped (FDPE wins).",
            "Weight contract: Q16.16 unsigned with global infinity sentinel 0xFFFFFFFF.",
        ],
    }
    summary_path = out_dir / "skag_vector_summary.json"
    summary_path.write_text(json.dumps(summary, indent=2))

    print(f"Wrote {len(vectors)} SKAG vectors to {memh_path}")
    print(f" - {csv_path}")
