...
def main():
    ...
    csv_path = out_dir / 'qflow_top_tc2_candidates.csv'
    memh_path = tb_dir / 'qflow_top_tc2_candidates.memh'
    expected_path = out_dir / 'qflow_top_tc2_expected.json'
    ...
    expected = {
        'integration_stage': 'step9c_top_tc_smoke',
        ...
        'notes': [
            'Step 9C smoke test validates the timing-closure-2 top using skag_mem_tc with stored GA weights.',
            'Candidate stream remains deterministic for controlled integration checking.'
        ]
    }
    expected_path.write_text(json.dumps(expected, indent=2))

    print(f'Wrote {len(rows)} top-level timing-closure-2 candidates to {memh_path}')
    print(f' - {csv_path}')
