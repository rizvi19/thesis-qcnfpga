...
def main():
    ...
    csv_path = out_dir / 'qflow_top_candidates.csv'
    memh_path = tb_dir / 'qflow_top_candidates.memh'
    expected_path = out_dir / 'qflow_top_expected.json'
    ...
    expected = {
        'integration_stage': 'step7_top_smoke',
        ...
        'notes': [
            'Step 7 smoke test wires PRNG + FDPE + SKAG + PMO-GA multigen in one simulation.',
            'Candidate stream remains deterministic for controlled integration checking.'
        ]
    }
    expected_path.write_text(json.dumps(expected, indent=2))

    print(f'Wrote {len(rows)} top-level smoke candidates to {memh_path}')
    print(f' - {csv_path}')
